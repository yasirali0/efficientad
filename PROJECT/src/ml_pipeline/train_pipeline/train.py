...
from ..models.efficientad import Teacher, Student, AutoEncoder
from ...data_pipeline.datasets import get_dataset, infinite_dataloader
...
class Trainer(object):
    def __init__(self, config):
        ...
        self.teacher_ckpt = config['Model']['checkpoints']
    ...
    def train(self, iterations=70000):
        ...
        optimizer = optim.Adam(list(self.student.parameters()) + list(self.ae.parameters()), lr=1e-4, weight_decay=1e-5)
        scheduler = torch.optim.lr_scheduler.StepLR(
            optimizer, step_size = int(0.95 * iterations), gamma=0.1)
        
        best_auroc = 0
        best_loss = 100
        logger.debug('start train iter:{}'.format(iterations))
        for i_batch in range(iterations):
            ...
            if i_batch % self.print_freq == 0:
                ...
                logger.debug("label:{}, batch:{}/{}, loss_total:{:.4f}, loss_st:{:.4f}, loss_ae:{:.4f}, loss_stae:{:.4f}".format(
                ...
                if loss_total < best_loss:
                    ...
                    if auroc > best_auroc:
                        ...
                        logger.debug('saving model in {} at auroc:{:.4f}'.format(self.ckpt_dir, auroc))
    ...
    def load_pretrain_teacher(self):
        self.teacher.load_state_dict(torch.load(self.teacher_ckpt))
        self.teacher = self.teacher.cuda()
        self.teacher.eval()
        for parameters in self.teacher.parameters():
            parameters.requires_grad = False
        print('load teacher model from {}'.format(self.teacher_ckpt))
    ...
    def loss_st(self, image, imagenet_iterator, teacher:Teacher, student:Student):
        ...
        s_imagenet_out = student(image_p['image'][0].cuda())
    ...
    def caculate_channel_std(self,dataloader):
        ...
        self.channel_mean, self.channel_std = global_channel_normalize(dataloader, self.teacher, normalize_iter=1000, channel_size=self.channel_size, resize=self.resize)
        print('channel mean:{}'.format(self.channel_mean.shape),'channel std:{}'.format(self.channel_std.shape))
        logger.debug('channel mean:{}'.format(self.channel_mean.shape), 'channel std:{}'.format(self.channel_std.shape))
        channel_std = {
    ...
    def load_datasets(self):
        ...
        logger.debug('load train dataset:length:{}'.format(len(dataset)))
        
        quantile_dataset = get_dataset(
                        type=self.config['Datasets']['train']['type'],
                        root=self.config['Datasets']['train']['root'],
                        transform=self.data_transforms,
                        gt_transform=self.gt_transforms,
        ...
        imagenet_iterator = infinite_dataloader(imagenet_loader)
...
if __name__ == '__main__':
    ...
    config = get_config(dataset_name='MVTec_AD')

    if not os.path.exists(config['ckpt_dir']):
        os.makedirs(config['ckpt_dir'])

    efficientad_trainer = Trainer(
        config=config
    )

...
class Inference(object):
    ...
    def __init__(self, category, val_dir, model_path, ratio=0.1, score_in_mid_size=224, channel=384, result_path = 'data/result', model_size='S', resize=256, device='cuda', dataset_type='MVTec') -> None:
        ...
        self.result_path = os.path.join(result_path, category)
    ...
    def eval(self):
        ...
        logger.debug(f"result path: {self.result_path}")
        ...
        for i_batch, sample_batched in tqdm(enumerate(dataloader)):
            ...
            out_hstack = np.hstack((origin_img_np, gt_rgb, origin_with_fmap, out_im_thresh))
        ...
        if total_gt_pixel_scoresnp.max() == 0:
            logger.debug("label:{}, auroc:{:.4f}".format(self.category, auroc))
            return
        ...
        logger.debug("label:{}, auroc:{:.4f}, auroc_pixel:{:.4f}, ap:{:.4f}, ap_pixel:{:.4f}".format(self.category, auroc, auroc_pixel, ap, ap_pixel))    
